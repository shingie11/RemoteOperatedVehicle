...
int read_image(Ref<LuminanceSource> source, bool hybrid, string expected) {
  ...
  try {
	...
	/*int sz,j; 
	Ref<BitMatrix> blMat=binary->getBlackMatrix();
    Ref<BitArray> blRow(new BitArray(blMat->getWidth()));
    for (int i = 0; i< blMat->getHeight(); i+=2) {
        blRow = blMat->getRow(i, blRow); 
        cout << " [ " ;
        for (j = 0, sz = blRow->getSize(); j < sz; j+=2) {
            cout << (blRow->get(j) ? 'X' : '.');
            cout << " ";
        }      
	    cout << " ]" <<endl;
