...
ArrayRef<char> BitMatrixParser::readCodewords() {
  ...
  /* Log data to a test file */
	FILE *fplogfile, *fptxtlogfile;
	fptxtlogfile=fopen("Decoderlogdata.txt","w");
    for (int counter = 0; counter < version->getTotalCodewords(); counter++)
    {
	    for (int bit =0 ; bit < 8 ; bit++)
	    {
		    if (result[counter] & ( 1<<(7-bit))) fprintf(fptxtlogfile,"1"); else fprintf(fptxtlogfile,"0");
	    }
    }
  return result;
}
